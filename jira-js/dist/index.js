...
function get(path, params) {
    const url = params
        ? path + "?" + new URLSearchParams(Object.fromEntries(Object.entries(params).map(([k, v]) => [k, String(v)]))).toString()
        : path;
    return jiraFetch(url);
}
...
function extractText(doc) {
    ...
    if (Array.isArray(doc)) {
        return doc.map(extractText).filter(Boolean).join("\n");
    }
    return "";
}
...
// Tool: search_issues
// ---------------------------------------------------------------------------
server.tool("search_issues", "Search JIRA issues using JQL (Jira Query Language).", {
...
// Tool: get_issue
// ---------------------------------------------------------------------------
server.tool("get_issue", "Get full details of a JIRA issue by its key (e.g. PROJ-123).", {
    issue_key: z.string().describe('The issue key, e.g. "PROJ-123"'),
}, async ({ issue_key }) => {
    const issue = await get(`/rest/api/3/issue/${issue_key}`);
    const result = formatIssue(issue);
    const commentsRaw = (issue.fields["comment"] ?? {}).comments ?? [];
...
server.tool("create_issue", "Create a new JIRA issue.", {
    ...
    const result = await post("/rest/api/3/issue", { fields });
...
// Tool: update_issue
// ---------------------------------------------------------------------------
server.tool("update_issue", "Update fields on an existing JIRA issue.", {
    issue_key: z.string().describe('The issue key, e.g. "PROJ-123"'),
...
server.tool("add_comment", "Add a comment to a JIRA issue.", {
    ...
    const result = await post(`/rest/api/3/issue/${issue_key}/comment`, { body: textToAdf(body) });
    const out = {
        id: result["id"],
...
// ---------------------------------------------------------------------------
server.tool("transition_issue", "Change the status of a JIRA issue by performing a named transition.", {
    issue_key: z.string().describe('The issue key, e.g. "PROJ-123"'),
    transition_name: z.string().describe('Transition name, e.g. "In Progress", "Done"'),
}, async ({ issue_key, transition_name }) => {
    const data = await get(`/rest/api/3/issue/${issue_key}/transitions`);
    ...
    if (!match) {
        ...
        throw new Error(`Transition '${transition_name}' not found. Available: ${available.join(", ")}`);
    }
...
// ---------------------------------------------------------------------------
server.tool("get_transitions", "List available transitions (status changes) for a JIRA issue.", {
    issue_key: z.string().describe('The issue key, e.g. "PROJ-123"'),
}, async ({ issue_key }) => {
    const data = await get(`/rest/api/3/issue/${issue_key}/transitions`);
...
// Tool: assign_issue
// ---------------------------------------------------------------------------
server.tool("assign_issue", 'Assign a JIRA issue to a user. Use "-1" for automatic assignment or "none" to unassign.', {
    issue_key: z.string().describe('The issue key, e.g. "PROJ-123"'),
...
// Tool: list_projects
// ---------------------------------------------------------------------------
server.tool("list_projects", "List JIRA projects accessible to the authenticated user.", {
    max_results: z.number().int().min(1).max(100).default(50).describe("Maximum projects to return"),
}, async ({ max_results }) => {
    const data = await get("/rest/api/3/project/search", { maxResults: Math.min(max_results, 100) });
...
// Tool: get_project
// ---------------------------------------------------------------------------
server.tool("get_project", "Get details of a specific JIRA project.", {
    project_key: z.string().describe('The project key, e.g. "PROJ"'),
}, async ({ project_key }) => {
    const p = await get(`/rest/api/3/project/${project_key}`);
    const out = {
        key: p.key,
        name: p.name,
...
// Tool: list_boards
// ---------------------------------------------------------------------------
server.tool("list_boards", "List agile boards, optionally filtered by project.", {
    ...
    const data = await get("/rest/agile/1.0/board", params);
...
// Tool: list_sprints
// ---------------------------------------------------------------------------
server.tool("list_sprints", "List sprints for an agile board.", {
    ...
    const data = await get(`/rest/agile/1.0/board/${board_id}/sprint`, { state, maxResults: 50 });
...
// ---------------------------------------------------------------------------
server.tool("get_sprint_issues", "List issues in a specific sprint.", {
    ...
    const data = await get(`/rest/agile/1.0/sprint/${sprint_id}/issue`, { maxResults: Math.min(max_results, 100) });
...
// Tool: search_users
// ---------------------------------------------------------------------------
server.tool("search_users", "Search for JIRA users by name or email.", {
    ...
    const users = await get("/rest/api/3/user/search", { query, maxResults: Math.min(max_results, 50) });
...
// Tool: link_issues
// ---------------------------------------------------------------------------
server.tool("link_issues", 'Create a link between two JIRA issues (e.g. "Blocks", "Relates", "Duplicate").', {
...
// Tool: add_worklog
// ---------------------------------------------------------------------------
server.tool("add_worklog", "Log work against a JIRA issue.", {
    issue_key: z.string().describe('The issue key, e.g. "PROJ-123"'),
    ...
    const payload = { timeSpent: time_spent };
    if (comment)
        payload["comment"] = textToAdf(comment);
    const result = await post(`/rest/api/3/issue/${issue_key}/worklog`, payload);
    const out = {
        id: result["id"],
